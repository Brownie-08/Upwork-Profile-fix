...
def profile(request):
    ...
    operator_documents = OperatorDocument.objects.filter(user=request.user)
...
def add_education(request):
    ...
    if (
        ...
        logger.warning(f"Invalid education request by {request.user.username}")
...
def add_portfolio(request):
    ...
    if (
        ...
        if portfolio_form.is_valid() and sample_form.is_valid():
            ...
            errors = {
                "portfolio_errors": portfolio_form.errors.as_json(),
                "sample_errors": sample_form.errors.as_json(),
            }
            logger.warning(
        ...
        logger.warning(f"Invalid portfolio request by {request.user.username}")
        return JsonResponse({"error": "Invalid request"}, status=400)
...
def edit_portfolio(request, pk):
    ...
    if request.method == "POST":
        ...
        portfolio.role = request.POST.get("role", portfolio.role)
        portfolio.skills = request.POST.get("skills", portfolio.skills)
        portfolio.related_job = request.POST.get("related_job", portfolio.related_job)
        ...
        if "video" in request.FILES:
            if sample.video:
                sample.video.delete(save=False)
            sample.video = request.FILES["video"]
        if "audio" in request.FILES:
            if sample.audio:
                sample.audio.delete(save=False)
            sample.audio = request.FILES["audio"]
        if "pdf" in request.FILES:
            if sample.pdf:
                sample.pdf.delete(save=False)
            sample.pdf = request.FILES["pdf"]
        sample.url = request.POST.get("url", sample.url)
        sample.text_block = request.POST.get("text_block", sample.text_block)
        sample.save()
        if not portfolio.sample:
            portfolio.sample = sample
            portfolio.save()
        ...
        return redirect("profiles:profile")
    return render(request, "profiles/edit_portfolio.html", {"portfolio": portfolio})
...
def update_education(request):
    ...
    if (
        ...
        if not edu_id:
            logger.warning(
                f"Missing education ID for update by {request.user.username}"
            )
            return JsonResponse(
                {"success": False, "error": "Missing education ID"}, status=400
            )

        education = get_object_or_404(
            Education, id=edu_id, profile=request.user.profile
        )
        ...
        logger.warning(f"Invalid education update request by {request.user.username}")
        return JsonResponse({"error": "Invalid request"}, status=400)
...
def add_experience(request):
    ...
    if (
        ...
        logger.warning(f"Invalid experience request by {request.user.username}")
        return JsonResponse({"error": "Invalid request"}, status=400)
...
def update_experience(request):
    ...
    if (
        ...
        if not exp_id:
            logger.warning(
                f"Missing experience ID for update by {request.user.username}"
            )
            return JsonResponse(
                {"success": False, "error": "Missing experience ID"}, status=400
            )
        experience = get_object_or_404(
            Experience, id=exp_id, profile=request.user.profile
        )
        ...
        logger.warning(f"Invalid experience update request by {request.user.username}")
        return JsonResponse({"error": "Invalid request"}, status=400)
...
def delete_experience(request, pk):
    ...
    if request.method == "POST":
        ...
        return redirect("profiles:profile")
    return render(
        request, "profiles/delete_experience.html", {"experience": experience}
    )
...
def delete_portfolio(request, pk):
    ...
    if request.method == "POST":
        ...
        if request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse({"success": True})
        return redirect("profiles:profile")
    return render(request, "profiles/delete_portfolio.html", {"portfolio": portfolio})
